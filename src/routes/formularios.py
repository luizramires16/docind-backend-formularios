...
def processar_orcamento():
    ...
    try:
        ...
        servicos = []
        if dados.get('servico_manuais'): servicos.append('Manuais Técnicos Personalizados')
        if dados.get('servico_fmea'): servicos.append('Análise de Falhas Estratégica (FMEA)')
        if dados.get('servico_manutencao'): servicos.append('Gestão de Manutenção Inteligente')
        if dados.get('servico_qualidade'): servicos.append('Documentação de Qualidade e Conformidade')
        if dados.get('servico_consultoria'): servicos.append('Consultoria Estratégica em Documentação')
        if dados.get('servico_implantacao'): servicos.append('Implantação de Sistemas e Treinamentos')
